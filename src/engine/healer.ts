...
const newLines = lines.map(line => {
  ...
  if (parts.length >= 2 && (parts[0] === 'pick' || parts[0] === 'merge')) {
    const hash = parts[1];
...
`;
    ...
    try {
      ...
      // Find the oldest violating commit to rebase from
      const oldestViolating = [...commits].reverse().find((c) => violatingHashes.has(c.hash));
      if (!oldestViolating) return true;

      let rebaseBase = `${oldestViolating.hash}^`;
      try {
        GitExec.run(`git rev-parse --verify "${rebaseBase}"`, {}, cwd);
      } catch {
        rebaseBase = '--root';
      }
      ...
      const rebaseCmd = rebaseBase === '--root'
        ? 'git rebase -i --rebase-merges --empty=drop --root'
        : `git rebase -i --rebase-merges --empty=drop ${rebaseBase}`;

      try {
        GitExec.run(rebaseCmd, env, cwd);
      } catch (rebaseMergesErr) {
        // Fallback without --rebase-merges if git version is older
        const fallbackCmd = rebaseBase === '--root'
          ? 'git rebase -i --root'
          : `git rebase -i ${rebaseBase}`;
        GitExec.run(fallbackCmd, env, cwd);
      }
